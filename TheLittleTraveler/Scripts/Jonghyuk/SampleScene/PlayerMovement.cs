    ...
    private void OnTriggerEnter(Collider starcoin)
    {
        if(starcoin.gameObject.CompareTag("PickUp"))
        {
            ...
            if(PlayerQuest_Desert.Desert)
            {
                ...
                {
                    GameObject.Find("PlayerTest").GetComponent<PlayerQuest_Desert>().starHint_desert.text = "<color=#FFFE77>" + "사막 " + CountingStar.star_cnt_desert + "/10" + "</color>";
                }
                else
                {
                    GameObject.Find("PlayerTest").GetComponent<PlayerQuest_Desert>().starHint_desert.text = "<color=#78FF79>" + "사막의 별 모으기 완료!" + "</color>";
                }
            }
            if(PlayerQuest_Forest.Forest)
            {
                ...
                {
                    GameObject.Find("Player").GetComponent<PlayerQuest_Forest>().starHint_forest.text = "<color=#FFFE77>" + "초원 " + CountingStar.star_cnt_forest + "/7" + "</color>";
                }
                else
                {
                    GameObject.Find("Player").GetComponent<PlayerQuest_Forest>().starHint_forest.text = "<color=#78FF79>" + "초원의 별 모으기 완료!" + "</color>";
                }
            }
            //countText.text = "Count:" + count.ToString();
